    ...
    if (typeof this.ordini === 'undefined') {
      this.renderChart({
        ...
        datasets: [{
          ...
          backgroundColor: ["#ffed67", "#00328a", "#6b9800", "#007df3", "#d80000", "#00beff", "#f00389", "#0078e7", "#d890ce", "#c15fbe"],
          data: this.incassi
        }]
      }, this.structure);
    } else {
...
var vm = new Vue({
  ...
  methods: {
    popolaPeriodo: function popolaPeriodo(anno, variable) {
      this[variable] = [];
      this.filter = [];
      this.numeroOrdini = [];
      this.incassiTotali = [];
      ...
      for (var data in this.response) {
        if (anno) {
          var split = this.response[data].created_at.split('-');
    ...
    countData: function countData(variable, data, element) {
      var _this = this;
      ...
      if (!this[variable].includes(element)) {
        ...
        this.filter.forEach(function (array_element) {
          if (array_element.data === element) {
            array_element.volte += 1;
            array_element.totale += parseInt(_this.response[data].total_price);
          }
        });
      }
    },
    makeFilter: function makeFilter() {
      ...
      this.filter.forEach(function (element) {
        _this2.numeroOrdini.push(element.volte);

        _this2.incassiTotali.push(element.totale);
      });
    },
      ...
      return _babel_runtime_regenerator__WEBPACK_IMPORTED_MODULE_0___default.a.wrap(function _callee$(_context) {
        while (1) {
          switch (_context.prev = _context.next) {
            case 0:
              _this3.id = parseInt(window.location.href.split('/')[4]);
              _context.next = 3;
              return axios.get('/api/admin/' + _this3.id + '/statistics').then(function (response) {
                _this3.response = response.data;
              }, function (error) {
                _this3.responseError = error;
              });

            case 3:
              if (_.size(_this3.response) >= 1) {
                _this3.switchMonth();
              } else {//TODO inserire l'errore
              }
